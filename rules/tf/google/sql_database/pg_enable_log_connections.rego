# Adapted from https://github.com/fugue/regula (FG_R00425).
# Ported to the Vulnetix Rego input schema (input.file_contents).

package vulnetix.rules.fugue_tf_gcp_sql_pg_enable_log_connections
...
metadata := {
	"id": "FUGUE-TF-GCP-SQL-07",
	"name": "PostgreSQL database instance 'log_connections' database flag should be set to 'on'",
...
findings contains finding if {
	...
	not _flag_equals(r.block, "log_connections", "on")
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("google_sql_database_instance %q (PostgreSQL) does not set log_connections = \"on\".", [r.name]),
		"artifact_uri": r.path,
