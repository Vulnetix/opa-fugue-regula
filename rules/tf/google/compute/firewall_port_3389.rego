# Adapted from https://github.com/fugue/regula (FG_R00408).
# Ported to the Vulnetix Rego input schema (input.file_contents).

package vulnetix.rules.fugue_tf_gcp_gce_firewall_port_3389
...
metadata := {
	"id": "FUGUE-TF-GCP-GCE-06",
	"name": "Network firewall rules should not permit ingress from 0.0.0.0/0 to port 3389 (RDP)",
	...
	"cvssv4": "",
	"cwss": "",
	"tags": ["terraform", "gcp", "compute", "firewall", "rdp"],
}

findings contains finding if {
	some r in tf.resources("google_compute_firewall")
	_allows_world_on_port(r.block, "3389")
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("google_compute_firewall %q permits ingress from 0.0.0.0/0 to port 3389.", [r.name]),
		"artifact_uri": r.path,
