...
package vulnetix.rules.fugue_tf_az_pg_log_connections
...
metadata := {
	"id": "FUGUE-TF-AZ-PG-03",
	"name": "PostgreSQL Database configuration 'log_connections' should be on",
	"description": "PostgreSQL Database configuration 'log_connections' should be on. Enabling log_connections helps PostgreSQL Database log attempted and successful connections.",
...
findings contains finding if {
	...
	not _valid_config(s.name, "log_connections")
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("PostgreSQL server %q has log_connections explicitly set to 'off'.", [s.name]),
		"artifact_uri": s.path,
