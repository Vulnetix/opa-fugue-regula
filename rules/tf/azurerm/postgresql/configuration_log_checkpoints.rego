...
package vulnetix.rules.fugue_tf_az_pg_log_checkpoints
...
metadata := {
	"id": "FUGUE-TF-AZ-PG-02",
	"name": "PostgreSQL Database configuration 'log_checkpoints' should be on",
	"description": "PostgreSQL Database configuration 'log_checkpoints' should be on. Enabling log_checkpoints helps the PostgreSQL Database log each checkpoint which generates query and error logs.",
...
findings contains finding if {
	...
	not _valid_config(s.name, "log_checkpoints")
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("PostgreSQL server %q has log_checkpoints explicitly set to 'off'.", [s.name]),
		"artifact_uri": s.path,
