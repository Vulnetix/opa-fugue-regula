...
package vulnetix.rules.fugue_tf_az_net_inbound_22
...
metadata := {
	"id": "FUGUE-TF-AZ-NET-03",
	"name": "NSGs should not permit ingress from 0.0.0.0/0 to TCP/UDP port 22 (SSH)",
	"description": "Virtual Network security groups should not permit ingress from '0.0.0.0/0' to TCP/UDP port 22 (SSH). The potential security problem with using SSH over the internet is that attackers can use various brute force techniques to gain access to Azure Virtual Machines.",
	...
	"cvssv4": "",
	"cwss": "",
	"tags": ["terraform", "azure", "network", "ssh"],
}

findings contains finding if {
	some r in tf.resources("azurerm_network_security_rule")
	_bad_inbound(r.block, 22)
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("NSG rule %q permits inbound SSH (port 22) from 0.0.0.0/0.", [r.name]),
		"artifact_uri": r.path,
...
findings contains finding if {
	...
	_bad_inbound(rule, 22)
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("NSG %q has an inline security_rule permitting SSH (port 22) from 0.0.0.0/0.", [sg.name]),
