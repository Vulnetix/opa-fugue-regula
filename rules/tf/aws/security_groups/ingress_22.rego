...
# Simplified: flags aws_security_group with ingress covering port 22 from 0.0.0.0/0.

package vulnetix.rules.fugue_tf_aws_sg_13
...
metadata := {
	"id": "FUGUE-TF-AWS-SG-13",
	"name": "Security group rules should not permit ingress from 0.0.0.0/0 to port 22 (SSH)",
	"description": "Unrestricted ingress from the internet to port 22 exposes SSH to remote attack.",
...
findings contains finding if {
	...
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("aws_security_group %q allows ingress from 0.0.0.0/0 to port 22.", [sg.name]),
...
findings contains finding if {
	...
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("aws_security_group_rule %q allows ingress from 0.0.0.0/0 to port 22.", [r.name]),
		"artifact_uri": r.path,
		"severity": metadata.severity,
		"level": metadata.level,
		"start_line": 1,
