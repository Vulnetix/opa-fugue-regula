...
# Simplified: flags aws_security_group with ingress covering port 8000 from 0.0.0.0/0.

package vulnetix.rules.fugue_tf_aws_sg_35
...
metadata := {
	"id": "FUGUE-TF-AWS-SG-35",
	"name": "Security group rules should not permit ingress from 0.0.0.0/0 to port 8000 (HTTP dev)",
	"description": "Unrestricted ingress from the internet to port 8000 exposes HTTP dev to remote attack.",
...
findings contains finding if {
	...
	_covers_port(rule, 8000)
	_rule_zero_cidr(rule)
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("aws_security_group %q allows ingress from 0.0.0.0/0 to port 8000.", [sg.name]),
...
findings contains finding if {
	...
	_covers_port(r.block, 8000)
	_rule_zero_cidr(r.block)
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("aws_security_group_rule %q allows ingress from 0.0.0.0/0 to port 8000.", [r.name]),
		"artifact_uri": r.path,
		"severity": metadata.severity,
		"level": metadata.level,
		"start_line": 1,
