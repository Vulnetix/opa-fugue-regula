...
# Simplified: flags aws_security_group with ingress covering port 3389 from 0.0.0.0/0.

package vulnetix.rules.fugue_tf_aws_sg_25
...
metadata := {
	"id": "FUGUE-TF-AWS-SG-25",
	"name": "Security group rules should not permit ingress from 0.0.0.0/0 to port 3389 (RDP)",
	"description": "Unrestricted ingress from the internet to port 3389 exposes RDP to remote attack.",
...
findings contains finding if {
	...
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("aws_security_group %q allows ingress from 0.0.0.0/0 to port 3389.", [sg.name]),
...
findings contains finding if {
	...
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("aws_security_group_rule %q allows ingress from 0.0.0.0/0 to port 3389.", [r.name]),
		"artifact_uri": r.path,
		"severity": metadata.severity,
		"level": metadata.level,
		"start_line": 1,
