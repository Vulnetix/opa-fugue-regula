...
# Simplified: flags aws_security_group with ingress covering port 3306 from 0.0.0.0/0.

package vulnetix.rules.fugue_tf_aws_sg_24
...
metadata := {
	"id": "FUGUE-TF-AWS-SG-24",
	"name": "Security group rules should not permit ingress from 0.0.0.0/0 to port 3306 (MySQL)",
	"description": "Unrestricted ingress from the internet to port 3306 exposes MySQL to remote attack.",
...
findings contains finding if {
	...
	_covers_port(rule, 3306)
	_rule_zero_cidr(rule)
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("aws_security_group %q allows ingress from 0.0.0.0/0 to port 3306.", [sg.name]),
...
findings contains finding if {
	...
	_covers_port(r.block, 3306)
	_rule_zero_cidr(r.block)
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("aws_security_group_rule %q allows ingress from 0.0.0.0/0 to port 3306.", [r.name]),
		"artifact_uri": r.path,
		"severity": metadata.severity,
		"level": metadata.level,
		"start_line": 1,
