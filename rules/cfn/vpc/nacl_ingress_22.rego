...
package vulnetix.rules.fugue_cfn_vpc_nacl_ingress_22
...
metadata := {
	"id": "FUGUE-CFN-VPC-05",
	"name": "VPC NACLs should not allow 0.0.0.0/0 ingress to port 22",
	"description": "VPC network ACLs should not allow ingress from 0.0.0.0/0 to port 22. Public access to remote server administration ports such as 22 and 3389 increases resource attack surface.",
	...
	"cvssv4": "",
	"cwss": "",
	"tags": ["cloudformation", "aws", "vpc", "nacl", "ssh"],
}
...
findings contains finding if {
	...
	_nacl_allows_port(r, 22)
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("NetworkAcl %q has an entry allowing 0.0.0.0/0 ingress to port 22.", [r.logical_id]),
		"artifact_uri": r.path,
