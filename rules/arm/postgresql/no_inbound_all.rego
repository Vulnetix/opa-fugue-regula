...
package vulnetix.rules.fugue_arm_postgresql_no_inbound_all
...
metadata := {
	"id": "FUGUE-ARM-PG-08",
	"name": "PostgreSQL Database server firewall rules should not permit start and end IP addresses to be 0.0.0.0",
	...
	"cvssv4": "",
	"cwss": "",
	"tags": ["arm", "azure", "postgresql", "firewall"],
}

findings contains finding if {
	some r in arm.resources("Microsoft.DBforPostgreSQL/servers/firewallRules")
	r.resource.properties.startIpAddress == "0.0.0.0"
	r.resource.properties.endIpAddress == "0.0.0.0"
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("PostgreSQL firewall rule %q allows all Azure services (0.0.0.0-0.0.0.0).", [r.resource.name]),
		"artifact_uri": r.path,
