...
package vulnetix.rules.fugue_arm_postgresql_log_disconnections
...
_ok(server) if {
	lower(_config_value(server, "log_disconnections")) == "on"
}
...
findings contains finding if {
	...
	finding := {
		"rule_id": metadata.id,
		"message": sprintf("PostgreSQL server %q does not have log_disconnections=on.", [s.resource.name]),
		"artifact_uri": s.path,
